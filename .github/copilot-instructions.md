...
Use `docs/vega-lite/projected-balance.vl.json` as the reference layout for
projected balance experiments in the online Vega editor. The x-axis is ordinal
by anchor date so each anchor day appears once instead of using a continuous
monthly time scale. Inline values are sample-shaped data; replace them with
forecast anchor rows.
