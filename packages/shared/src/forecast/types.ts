 ...
 *                    first billing day on or after the entry date (a charge
 *                    dated on the billing day settles on that same day's bill).
 * - 'savings:<potId>' a transfer between the bank and a savings pot. It hits
 *                    the bank on its own date (like a debit card) and moves
 *                    the opposite way in the pot: a contribution is negative
 *                    (bank down, pot up), a withdrawal is positive (bank up,
 *                    pot down).
 */
export type Channel = 'bank' | `cc:${string}` | `savings:${string}`;

export const CC_CHANNEL_PREFIX = 'cc:';
export const SAVINGS_CHANNEL_PREFIX = 'savings:';

/** Card id for a `cc:` channel, else null. */
export const cardIdOfChannel = (channel: Channel): string | null =>
  channel.startsWith(CC_CHANNEL_PREFIX) ? channel.slice(CC_CHANNEL_PREFIX.length) : null;

/** Pot id for a `savings:` channel, else null. */
export const potIdOfChannel = (channel: Channel): string | null =>
  channel.startsWith(SAVINGS_CHANNEL_PREFIX)
    ? channel.slice(SAVINGS_CHANNEL_PREFIX.length)
    : null;

/** Channel string for a savings pot. */
export const savingsChannelOf = (potId: string): Channel =>
  `${SAVINGS_CHANNEL_PREFIX}${potId}`;
...
/**
 * A savings pot: money that has genuinely left the bank account into a
 * separate savings/deposit account. Contributions are ordinary ledger entries
 * (usually generated by a recurring template) on the pot's `savings:` channel,
 * so they reduce the projected bank balance like any other outflow while the
 * pot balance is projected forward alongside it.
 */
export interface SavingsPot {
  id: string;
  name: string;
  /** Balance held in the pot at end-of-day `asOf`. Non-negative. */
  balance: number;
  /** Snapshot date for `balance`. Must not be in the future. */
  asOf: IsoDate;
}
...
export type ChargeSource =
  | {
      ...
      occurrenceKey?: string;
      /**
       * Set IFF the entry is on a `savings:` channel. Lets the UI mark the
       * charge as a transfer into savings rather than money spent.
       */
      potId?: string;
    }
  | {
...
export interface PotDailyBalance {
  date: IsoDate;
  /** Projected pot balance at end of `date`. */
  balance: number;
  /** Net movement on `date`; positive = money added to the pot. */
  delta: number;
}
...
export interface PotForecast {
  ...
  name: string;
  asOf: IsoDate;
  /** Raw `balance` from the pot snapshot (held at end-of-day `asOf`). */
  snapshotBalance: number;
  /** Pot balance projected at the first emitted day (visibleStart). */
  openingBalance: number;
  /** Pot balance projected at the last emitted day (endDate). */
  closingBalance: number;
  /** Net amount added over the visible window (closing − opening). */
  contributedInWindow: number;
  /** Day-by-day pot balance over the forecast window. */
  days: PotDailyBalance[];
}
...
export interface ForecastResult {
  ...
  cards: CardForecast[];
  /** Projected savings pots, one per configured pot. Empty when none exist. */
  pots: PotForecast[];
