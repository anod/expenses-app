...
export interface BilledChargeRow {
  ...
  /** Signed installment full price, shown next to the X/Y badge. Null for
   * non-installment rows. */
...
export const buildForecastTimeline = ({
  ...
  /** Signed installment full price for the entry's template, or null when the
   * template is open-ended / non-monthly. Uses the stored `fullPrice`, falling
   * back to amount × scheduled-payment-count for templates seeded before the
   * full-price column existed. */
  const fullPriceForEntry = (entry: { recurringId?: string }): number | null => {
    if (!entry.recurringId) return null;
    const template = templatesById.get(entry.recurringId);
    if (!template || template.cadence.kind !== 'monthly' || template.endDate == null) return null;
    if (template.fullPrice != null) return template.fullPrice;
    const total = scheduledPaymentCount(template);
    return total ? roundCents(template.amount * total) : null;
  };
  ...
  const toChargeItem = (
    ...
    if (charge.source.kind === 'cc-bill') {
      ...
      const toBilledRow = (entry: LedgerEntry, accounted: boolean): BilledChargeRow => ({
        ...
        fullPrice: fullPriceForEntry(entry),
        ...(accounted ? { past: true } : {}),
